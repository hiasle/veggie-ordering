...
import {Customer} from "../../../../openapi/model/customer";
...
export class OrderFormComponent {
  ...
  async saveOrder(): Promise<void> {
    console.log('Customerfield: ', this.form.value.customer);
    console.log('Form valid: ', this.form.valid);
    console.log('Form valid: ', this.form.errors);
    if (!this.form.valid) {
      this.submitted = true;
      return;
    }
    const order = {
      customer: {
        id: this.form.value.customer?.id,
      },
    ...
    await this.router.navigate(['verkaeufe']);
  }
  ...
  displayFn(customer: any): string {
    ...
    return customer && customer.id ? `${customer.firstname} ${customer.lastname}` : '';
  }
